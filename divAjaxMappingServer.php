 ...
 *
 */
define("DIV_AJAX_MAPPING_ACCESS_DENIED_HOST", 
        "DIV_AJAX_MAPPING_ACCESS_DENIED_HOST");
define("DIV_AJAX_MAPPING_ACCESS_DENIED_USER", 
        "DIV_AJAX_MAPPING_ACCESS_DENIED_USER");
...
define("DIV_AJAX_MAPPING_LOGOUT_SUCCESSFUL", 
        "DIV_AJAX_MAPPING_LOGOUT_SUCCESSFUL");
define("DIV_AJAX_MAPPING_METHOD_EXECUTED", "DIV_AJAX_MAPPING_METHOD_EXECUTED");
define("DIV_AJAX_MAPPING_METHOD_NOT_EXISTS", 
        "DIV_AJAX_MAPPING_METHOD_NOT_EXISTS");

/**
 * How to use?
 *
 * $server = new divAjaxMappingServer(); // Server instance
...
class divAjaxMappingServer
{
     ...
     * @param array $hosts            
     */
    public function addMethod ($name, $params = array(), $params_complex = false, 
            $security = false, $hosts = array(), $include = null, $namespace = null)
    {
        if ($params == '')
            $params = array();
        ...
        foreach ($this->methods as $key => $m) {
            ...
            if (strpos($key, "::") !== false && $namespace === false) {
                ...
                if ($m['params_complex'] == true) {
                    $clases[$arr[0]] .= ($namespace !== false ? "'$namespace'" : "{$arr[1]}") .
                             ": function(params){\n";
                } else {
                    $clases[$arr[0]] .= ($namespace !== false ? "'$namespace'" : "{$arr[1]}") .
                             ": function(";
                ...
                $clases[$arr[0]] .= "\n   return divAjaxMapping.call(this.__server, '{$arr[0]}::{$arr[1]}',params);}";
            } else {
                
                echo $j ++ > 0 ? ", " : "";
                
                if ($m['params_complex'] == true) {
                    echo ($namespace !== false ? "'$namespace'" : "$key") .
                             ": function(params){\n";
                } else {
                    echo ($namespace !== false ? "'$namespace'" : "$key") .
                             ": function(";
                ...
                echo "\n    return divAjaxMappingServer.php.call(this.__server, '$key',params);\n }\n";
            }
        }
    ...
    private function execute ($method)
    {
        if (! isset($this->methods[$method]))
            ...
            // Execute hook before
        if (function_exists("divAjaxMapping_before")) {
            divAjaxMapping_before($method);
        }
        ...
        // Execute hook after
        if (function_exists("divAjaxMapping_after")) {
            divAjaxMapping_after($method);
        }
    ...
    final static function jsonEncode ($data)
    {
        if (is_array($data) || is_object($data)) {
            ...
            if ($islist)
                $json = '[' .
                         implode(',', 
                                array_map('divAjaxMappingServer::jsonEncode', 
                                        $data)) . ']';
            else {
                $items = array();
                foreach ($data as $key => $value) {
                    $items[] = self::jsonEncode("$key") . ':' .
    ...
    public function go ()
    {
        ...
        if (isset($_GET['login'])) {
            if (isset($_GET['password'])) {
                $r = divAjaxMapping::login($_GET['login'], $_GET['password']);
        ...
        if (isset($_GET['execute'])) {
            ...
            // Check host
            $ip = divAjaxMapping::getClientIPAddress();
            $hosts = $this->methods[$method]['hosts'];
            
            foreach ($hosts as $host) {
                $from = $host['from'];
                $to = $hots['to'];
                $v = divAjaxMapping::checkRangeIP($from, $to, $ip);
                if ($v === false) {
            ...
            $r = divAjaxMapping::checkMethodAccess($method, $namespace);
