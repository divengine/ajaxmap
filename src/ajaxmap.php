        ...
        foreach ($this->methods as $key => $m) {
            ...
            if (strpos($key, "::") !== false && $namespace === false) {
                ...
                if ($m['params_complex'] == true) {
                    $classes[$arr[0]] .= ($namespace !== false ? "'$namespace'" : "{$arr[1]}") . ": function(params){\n";
                } else {
                    $classes[$arr[0]] .= ($namespace !== false ? "'$namespace'" : "{$arr[1]}") . ": function(";

                    $i = 0;
                    foreach ($m['params'] as $p) {
                        if ($i++ > 0) $classes[$arr[0]] .= ",";
                        $classes[$arr[0]] .= "$p";
                    }
